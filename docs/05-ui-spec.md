...
### ヘッダー
- タイトル「Alpha Radar」の**左にアプリアイコン**（40px、`next/image`、`frontend/public/logo.png` ＝ `src-tauri/icons/128x128.png` のコピー）。装飾扱い（`alt=""`）でタイトルが名前を担う。右側に CSV/JSON 出力・⚙設定。
...
- **P6 Chart**（`frontend/app/chart/page.tsx`、`components/MultiPaneChart|MtfSummary`、Rust `commands/get_chart_data`）: lightweight-charts v5 の4ペイン（価格+EMAリボン/Supertrend/一目+BUY/SELLマーカー / MACD 4色ヒスト+線 / Squeeze 4色 / 合成スコア+しきい値線）+ MTF サマリー + 足切替。**全系列は Rust 計算**でリストのスコアと一致（ADR-06）。`attributionLogo` 有効。
  ...
  - **トグル状態の永続化（ADR-17）**: さらに **アプリを終了→再起動しても復元**される（`app_data_dir/ui_prefs.json`、`config.json` とは別ファイル）。`ScanProvider` が起動時に `get_ui_prefs` で読み込み、トグル変更のたびに `update_ui_prefs` で保存。読み込みは寛容（既知キーかつ boolean のみ採用／破損・不在は既定へ縮退）。**足種（日足/週足/月足）とレーダーのソート/フィルタは永続化しない**（セッション限り＝意図的スコープ）。
  - **キーボード操作**: チャート画面で **`←` または `Backspace`** でレーダーへ戻る（「← レーダーへ戻る」リンクと同一動作＝`router.push("/")`）。修飾キー（Ctrl/Alt/Meta/Shift）付きは無効、テキスト入力欄にフォーカス中は無効（**チェックボックス上では有効** — トグルをクリックした直後に押せる必要があるため）。
