...
- **P6 Chart**（`frontend/app/chart/page.tsx`、`components/MultiPaneChart|MtfSummary`、Rust `commands/get_chart_data`）: lightweight-charts v5 の4ペイン（価格+EMAリボン/Supertrend/一目+BUY/SELLマーカー / MACD 4色ヒスト+線 / Squeeze 4色 / 合成スコア+しきい値線）+ MTF サマリー + 足切替。**全系列は Rust 計算**でリストのスコアと一致（ADR-06）。`attributionLogo` 有効。
  ...
  - **表示トグル**: チャート上部にチェックボックス（EMAリボン / Supertrend / 一目 / MACD / Squeeze / **スコア** / 売買マーカー / Q-Trend / QT前兆 / **STフリップ**）。既定は一目均衡表と STフリップ が OFF。**チャート生成（`data` 依存）と表示切替（`visible` 依存）の useEffect を分離**し、トグル時は再生成せず `applyOptions({visible})` + ペイン stretch factor の更新のみ（ズーム維持）。
  ...
  - **STフリップ（ADR-16）**: Supertrend フリップに `ST LONG`（緑▲）/`ST SHORT`（赤▼）。ATS 視覚比較専用・単独エッジ実測なし（既定 OFF）。
